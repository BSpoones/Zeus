...
import org.bspoones.chloris.command.annotations.choices.ChannelTypes
        ...
        return when (parameterType) {
            ...
            User::class.java -> {
                if (!USER_PING_REGEX.matches(arg)) null
                    ...
                    CommandRegister.api.getUserById(id.toLong())
                }
            }
            Channel::class.java -> {
                if (!CHANNEL_PING_REGEX.matches(arg)) null
                    ...
                    val id = NUMBER_ONLY_REGEX.find(arg)?.value ?: return null
                    CommandRegister.api.getChannelById((parameterType as Channel)::class.java, id.toLong())
                }
            }
            Role::class.java -> {
                if (!ROLE_PING_REGEX.matches(arg)) null
                    ...
                    val id = NUMBER_ONLY_REGEX.find(arg)?.value ?: return null
                    CommandRegister.api.getRoleById(id.toLong())
                }
            }
            IMentionable::class.java -> {
                ...
                if (!roleMatch && !userMatch) null
                    ...
                    val id = NUMBER_ONLY_REGEX.find(arg)?.value ?: return null
                    if (roleMatch) CommandRegister.api.getRoleById(id.toLong())
                    else  CommandRegister.api.getUserById(id.toLong())
                }
            }
